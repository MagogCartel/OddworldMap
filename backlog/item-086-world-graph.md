...
- Layout is the hard part and hand-tuning is legitimate: a column per level, in progression order where the transition graph can derive it (the derivation [15](item-015-path-buttons-game-order.md) wants — build it once, both items consume it), paths stacked within their column, edges bundled. A force layout will produce spaghetti; don't start there.
