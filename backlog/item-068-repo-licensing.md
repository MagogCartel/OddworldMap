...
Whichever wins, establish what the root `LICENSE.txt` was originally intended to cover before codifying it. The vendored decoder is GPL-2.0 and the repository is GPL-2.0, and the relationship between those two facts should be confirmed rather than assumed.
