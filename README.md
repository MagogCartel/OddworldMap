...
- **Offscreen objects**: both games address their cameras on a grid slightly larger than the screen you see, and level authors sometimes ran an object into the leftover gap. The map lays screens out edge to edge, so that part of it lands over the neighbouring screen: an object's outline goes hollow and dotted exactly where it leaves its own screen, and stays solid and filled where the object really is. One with nothing on screen at all is dotted throughout, its tooltip says so, and its screen-list row is tagged `offscreen` — 276 of those in Oddysee, whose gap is 656 units wide, and none in Exoddus, whose 7 is too narrow to hide anything. The games render none of it either
