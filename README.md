...
- **where you are**: a chip at the top-left corner of the map names the level and path you are looking at; click it (or press **`i`**) for the level's full name, which half of it the path belongs to where the game says, whether the path is one the game arrives at from another level or one only the title-screen demos play, and the path's note where it has one — a remark on something odd about the place, marked by a dot on the chip
