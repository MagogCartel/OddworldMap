...
- level and path buttons top-left; object category filters with counts below — hover a path button for its area name: the in-game one where the game defines it (Rupture Farms Return's Zulag 1–4, Exoddus ender areas), otherwise a community name from [annotations.json](annotations.json)
