...
- **Saved on this device**
  ...
  - "Object edits" counts the objects you have edited on the map, across both games, and forgets them all in two presses. An edit is kept as its difference from the disc, so a rebuild of the map applies it where its object still is and forgets, with a note, any that no longer match; an embedded map applies none and always shows the disc's data
- **Buttons and names**
