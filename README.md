...
Everything under the repo root (`cams/`, `map_data_ao.js`, `map_data_ae.js`) is generated by [tools/build_map.py](tools/build_map.py) from raw PS1 disc images (2352-byte sectors, e.g. the `.bin` of a `.cue/.bin` dump):
...
- The viewer ([index.html](index.html)) is a single dependency-free HTML file; `map_data_ao.js` / `map_data_ae.js` carry the level/path/TLV/collision data as JSON.
