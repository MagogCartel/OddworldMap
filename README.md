...
- **hover** any object for decoded details
  ...
  - in the screen list and the Fields picker, a field name with a dotted underline carries a definition — what it means and its possible values, curated in [glossary.json](glossary.json); hover it with a mouse or tap it on a touch screen, where the tap still does what it would have done anyway (tick the field, jump to the object)
