...
// what an object anchored between camera windows has to say for itself
...
export const CATS = [
  { key:"board",  label:"LCD Status Boards", color:"#ff3860", on:true, names:["LCDStatusBoard"], tip:"The Mudokon tally boards: how many are left, saved and lost, and in Exoddus the ledger every rescue and death is booked against." },
  { key:"mud",    label:"Mudokons",          color:"#3ec6ff", on:true, names:["Mudokon","SlingMudokon","RingMudokon","LiftMudokon","MudokonPathTrans","TorturedMudokon"], tip:"Not only the workers Abe rescues: the chanters that grant spirit rings, the guards that sling unless their password is answered, the ones strung up in tear extractors, and the deaf ones placed as scenery." },
  { key:"door",   label:"Doors / Transitions", color:"#ffd23e", on:true, names:["Door","PathTransition","BirdPortal","BirdPortalExit","WellLocal","LocalWell","WellExpress","Teleporter","SlamDoor","MineCar","LevelLoader"], tip:"Doors, and everything else that carries Abe somewhere: path transitions, bird portals, wells, teleporters and the level loader." },
  { key:"cont",   label:"Continue points",   color:"#ffffff", on:true, names:["ContinuePoint","AbeStart","ElumStart"], tip:"Where the game restarts you, and the marks that place Abe at a level's start." },
  { key:"switch", label:"Switches / levers", color:"#5dde75", on:true, names:["Switch","Lever","InvisibleSwitch","FootSwitch","BellHammer","HandStone","IdSplitter","SecurityDoor","BellSongStone","ChimeLock","MovieHandStone","GlukkonSwitch","CrawlingSligButton","MultiSwitchController","WheelSyncer","TimerTrigger","WorkWheel","SlapLock","PullRingRope","ResetPath","ResetSwitchRange"], tip:"Everything that sets a switch id: levers, buttons, plates, work wheels, chime locks and hand stones, with the logic gates and resets that sit between them." },
  { key:"hazard", label:"Hazards",           color:"#ff8b3d", on:true, names:["DeathDrop","TimedMine","Mine","UXB","ElectricWall","MovingBomb","MeatSaw","DeathClock","GasEmitter","GasCountdown","TrapDoor","FallingItem","RollingBall","RollingRock","ZBall","Drill","LaughingGas","ExplosionSet","Water","SecurityOrb","SecurityClaw","MotionDetector"], tip:"Anything that hurts Abe, or fetches someone who will: mines, saws, drills, gas and water, plus the security orbs, claws and motion detectors that punish rather than kill." },
  { key:"enemy",  label:"Enemies / spawners", color:"#c85dff", on:true, names:["Slig","Slog","Paramite","Scrab","Bat","Bees","HoneySack","SligSpawner","SlogSpawner","ScrabSpawner","Glukkon","SlogHut","FlyingSlig","FlyingSligSpawner","CrawlingSlig","Fleech","Slurg","SlurgSpawner","ZzzSpawner","Greeter","SligGetPants","SligGetWings","BeeSwarmHole"], tip:"The creatures the level fields, hunting or not: Sligs, Slogs, Scrabs, Paramites, Fleeches and the rest, plus the spawners, marked spots that materialise fresh ones when a switch or timer asks." },
  { key:"pickup", label:"Pickups / dispensers", color:"#b6e83c", on:true, names:["RockSack","MeatSack","BoneBag","BoomMachine","BrewMachine"], tip:"What Abe can pick up: rock and meat sacks, bone bags, and the machines that dispense grenades and brew." },
  { key:"screen", label:"Displays",          color:"#3effc8", on:false, names:["LCDScreen","LCD","MovieStone","HintFly","DemoPlaybackStone","StatusLight","ColourfulMeter","DoorFlame"], tip:"The level's signs and readouts: LCD screens, hint flies, status lights, meters and door flames." },
  { key:"nav",    label:"Hoists / edges / lifts", color:"#8f9bb3", on:false, names:["Hoist","Edge","LiftPoint","LiftMover","Pulley","ElumWall","ScrabNoFall","RollingBallStopper","FlintLockFire","ParamiteWebLine"], tip:"The geometry that carries or stops a body: hoists, ledges, lifts and pulleys, and the invisible lines holding Paramites and Scrabs to their ground." },
  { key:"meta",   label:"Meta / bounds / fx", color:"#5b6270", on:false, names:[], tip:"Everything the other filters do not claim: the zones and bounds that shape behaviour without being seen, shadow and light effects, music triggers, and the game's own bookkeeping markers." },   // fallback for everything else
];
const NAME_CAT = {};
