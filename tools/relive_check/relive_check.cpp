...
// relive's own diagnostics are the only thing that names the TLV behind a
// WrongTLVLengthException, so they are kept on stdout; file output is turned off
// because ELPP_NO_DEFAULT_LOG_FILE leaves the logger with nowhere to write and
// the first message dereferences a null stream
