...
def bucket_cells(game_key, path, geometry):
    """authored grid cell -> the path's TLVs in list order. Which cell a rect
    ...
    cells = {}
    for t in path["tlvs"]:
        x = (t["x1"] + t["x2"]) // 2 if midpoint else t["x1"]
...
def export_path(game_key, game, rel, level, path, muds_in_level, links):
    ...
    buckets = bucket_cells(game_key, path, geometry)
    cameras = []
