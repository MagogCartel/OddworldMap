...
export let GEO = null, CELL_W = 368, CELL_H = 240;
let SX = 1, SY = 1;
...
export function dX(wx) { const c = Math.floor(wx / GEO.worldW); return c * CELL_W + (wx - c * GEO.worldW - GEO.winX) * SX; }
export function dY(wy) { const c = Math.floor(wy / GEO.worldH); return c * CELL_H + (wy - c * GEO.worldH - GEO.winY) * SY; }
export function wX(dx) { const c = Math.floor(dx / CELL_W); return c * GEO.worldW + GEO.winX + (dx - c * CELL_W) / SX; }
export function wY(dy) { const c = Math.floor(dy / CELL_H); return c * GEO.worldH + GEO.winY + (dy - c * CELL_H) / SY; }
